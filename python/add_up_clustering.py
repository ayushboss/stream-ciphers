...
for r in range(0, len(features)): # r represents the number of features we are adding
    ...
    for d in range(0, len(features)):
        ...
        testingNewMeasurements = clusterer.fit(best_features_df_trans)
        labelsOfNewMeasurements = testingNewMeasurements.labels_

        intra_cluster_distances_block = clusteringscoring.measurements_block(testingNewMeasurements, labelsOfNewMeasurements)
